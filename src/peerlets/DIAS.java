...
/**
 * This class is the core of DIAS, the Dynamic Efficient and Consistent Aggregation System. DIAS provides aggregation
 * services to different applications. Local stated are aggregated and the following aggregation functions are
 * calculated: SUM, AVG, SUM_SQR, MAX, MIN, COUNT, STDEV. Local possibleStates can change from a set of possible
 * possibleStates. The possible possibleStates of all the peers and their selections form an aggregation
 * aggregationEpoch.
 * 
 * DIAS differs from other aggregation mechanisms as it is only based on the concept of information dissemination.
 * Double-counting elements and changing local values are faced by a consistency mechanisms of mutual memberships
 * checks. This mechanisms is based on and implemented with bloom filters.
 *
 * Every DIAS host contains an aggregator and a disseminator. The disseminator contains the possisble possibleStates and
 * the selected selectedState. The aggregator keeps the aggregate of the aggregationEpoch.The mutual membership model
 * that DIAS exploits is the following:
 *
 * 1. Aggregators have memberships disseminators and vice versa 2. A possible selectedState has membership of
 * aggregators and a disseminator has memberships of aggregators. 3. An aggregate of an aggregator has memberships of
 * possibleStates and a disseminator has possible possibleStates selected by aggregators
 *
 * Based on this mutual memberships, consistency checks are performed that guarantee (probabilistically) that the
 * aggregation is updated and is free of double counts.
 *
 * The application can change the selected selectedState on-the-fly or even start a new aggregation aggregationEpoch by
 * providing new possible possibleStates and a selected selectedState.
 *
 * DIAS hosts communicate to disseminate information. The disseminator of a host contacts periodically the aggregator of
 * another host and provides the required information. The aggregator replies back to the disseminator. The roles
 * exchange in each host. The aggregator becomes disseminator and contacts the previous disseminator that is an
 * aggregator in this phase. The communications is realized in 3 messages:
 *
 * 1. Host A (Disseminator): Send PUSH to Host B (Aggregator) 2. Host B (Aggregator): Send PULL_PUSH to Host A
 * (Disseminator) 3. Host A (Aggregator): Send PULL to Host B (disseminator)
 *
 * Based on three exchanged messages, nodes mutually update their aggregates. Sampling of nodes is performed using the
 * Peer Sampling Service:
 *
 * M. Jelasity, S. Voulgaris, R. Guerraoui, A.-M. Kermarrec, M. van Steen., Gossip-based peer sampling, ACM Transactions
 * on Computer Systems, August 2007
 *
 * Other routing mechanisms can be used as well, e.g. random walks, flooding, etc.
 *
 * DIAS exploits routing mechanisms by employing aggregation strategies between three type of neighbors:
 *
 * 1. Unexploited: These neighbors have not contacted yet for an aggregation 2. Exploited: These neighbors have
 * contacted for an aggrgation and contain the most recent selected state 3. Outdated: These neighbors have an old
 * selected state.
 *
 ...
 *
 * 1. RANDOM: Either an outdated or an unexploited neighbor is selected for aggregation. If there is no available
 * neighbor in either of them, the second choice is tried 2. EXPLOITATION: Unexploited neighbors are selected. If there
 * is no any available, an outdated is selected. 3. UPDATE: An outdated neighbor is selected. If there is no any
 * available, an unexploited is selected.
 *
 * @author Evangelos
 */
public class DIAS extends BasePeerlet implements DIASInterface, AggregationInterface {
	...
	private static final Logger logger = Logger.getLogger(DIAS.class);

	/**
	 * DIAS initialization
	 *
	 * @param id
	 *            the local identifier of the experiment of a specific peer
	 * @param Tdias
	 *            the dissemination period
	 * @param numOfSessions
	 *            the number of sessions that can run periodically in DIAS
	 * @param Tsampling
	 *            the sampling period from Peer Sampling Service
	 * @param sampleSize
	 *            the sample size for selecting contact aggregators
	 * @param strategy
	 *            the strategy for the sampling selection of candidate aggregators
	 * @param undiscoveredSize
	 *            the size of the queue with the undiscovered neighbors
	 * @param outdatedSize
	 *            the size of the queue with the outdated neighbors
	 * @param exploitedSize
	 *            the size of the queue with the updated neighbors
	 * @param bloomFilterParams
	 *            the parameterization of the bloom filters in the aggregator and disseminator
	 */
	public DIAS(String id, int Tdias, int numOfSessions, int Tsampling, int sampleSize,
			AggregationStrategy.Strategy strategy, int unexploitedSize, int outdatedSize, int exploitedSize,
			Map<BloomFilterParams, Object> bloomFilterParams) {
		this.id = id;
		this.Tdias = Tdias;
	...
	/**
	 * The active selectedState of the DIAS peerlet is executed periodically. A peer is selected and a PUSH message is
	 * potentially sent for each aggregation aggregationEpoch.
	 */
	...
	/**
	 * A number of samples are collected and provided to the adaptation strategy. The method also provides the
	 * pullPushAck of the check in the AMD and AMS bloom filters given the collected sample and the selected state
	 * respectively.
	 */
			...
			public void timerExpired(Timer timer) {
				for (int i = 0; i < sampleSize; i++) {
					...
					if (!strategy.containsSample(sample)) {
						strategy.setSample(sample, disseminator.checkAMDMembership(sample),
	...
	/**
	 * The passive selectedState defines the reaction of a DIAS peerlet to a received DIAS message. A reaction is
	 * defined if and only if the two nodes aggregate within the same aggregationEpoch. Reactions are as follows:
	 *
	 * 1. PUSH: The aggregator report is generated and the disseminator report is potentially incorporated in the
	 * pull-push message. 2. PULL_PUSH: The disseminator receives the final aggregator report. The aggregator aggregates
	 * the dissemiantor report and a pull message is generated for the disseminator. 3. PULL: The diseminator receives
	 * the aggregator report.
	 *
	 * @param message
	 *            the received DIAS message
	 */
		...
		if (message.aggregationEpoch == this.aggregationEpoch) {
			...
			case PULL_PUSH:
				...
				logger.debug("ppidx " + getPeer().getIndexNumber() + " AggregateSum"
						...
						+ ((ArithmeticState) pullPush.push.report.get(DisseminatorReport.SELECTED_STATE)).getValue());
	...
	/**
	 * Handles incoming messages. Messages are processed if their type is DIAS and the peerlet is active.
	 *
	 * @param message
	...
	/**
	 * Creates a PUSH message by creating a dissemiantor report for a given aggregator
	 *
	 * @param aggregator
	 *            the aggregator for which the disseminator report is generated
	 *
	...
	/**
	 * Creates a PULL message by creating an aggregator report based on a received disseminator report
	 *
	 * @param disseminator
	 *            the disseminator that contacted the local aggregator
	 * @param disseminatorReport
	 *            the report received by the disseminator
	 *
	...
	private Pull createPullMessage(Finger disseminator, HashMap<DisseminatorReport, Object> disseminatorReport) {
		...
		HashMap<AggregatorReport, Object> aggregatorReport = aggregator.receiveDisseminatorReport(disseminator,
				disseminatorReport);
	...
	/**
	 * Creates a PULL-PUSH message by replying to a disseminator with the aggregator report and embeding a new PUSH
	 * message the disseminator report. In this way the aggregation is performed in two ways.
	 *
	 * @param disseminator
	 *            the disseeminator that contacted the local aggregator. Note that this is the case when the message is
	 *            perceived as a PULL. In the case of puss, it refers to the aggregator.
	 * @param disseminatorReport
	 *            the report received by the disseminator
	 *
	 ...
	 * 
	 * @param function
	 *            the aggregation function computed
	...
	/**
	 * Performs local aggregation between the aggregator and the disseminator in this peer.
	 */
	...
	/**
	 * This is how an application requests a new aggregation aggregationEpoch of a set of possible possibleStates a
	 * selected selectedState. If the DIAS peerlet is inactive, it is activated.
	 *
	...
	/**
	 * Application can change the selected selectedState dynamically and on-the-fly. The bloom filter memberships are
	 * adjusted to reflect the new local value in the aggregate. The aggregation strategy also rearranges the exploited,
		...
		getPeer().getMeasurementLogger().addMeasurementLoggerListener(new MeasurementLoggerListener() {
			public void measurementEpochEnded(MeasurementLog log, int epochNumber) {
				if (getPeer().getNetworkAddress().toString().equals("0")) {
					...
					Logger.getRootLogger().warn("Epoch " + epochNumber);
				}
				if (active) {
	...
	/**
	 * Counts the number of the outcomes per type for logging the values via the ProtoPeer measurements.
	 *
	 * @param outcome
